  ...
  return (
    <div>
      <Form style={{ margin: '6px' }}>
        ...
        <div>
          <label>Street: </label>
          <Field as={TextField} style={{ height: '25px', margin: '4px' }} name="street" />
          {touched.street && errors.street && <p>{errors.street}</p>}
        </div>
        <div>
          <label>City: </label>
          <Field as={TextField} style={{ height: '25px', margin: '4px' }} name="city" />
          {touched.city && errors.city && <p>{errors.city}</p>}
        </div>
        <div>
          <label>State: </label>
          <Field as={TextField} style={{ height: '25px', margin: '4px' }} name="state" />
          {touched.state && errors.state && <p>{errors.state}</p>}
        </div>
        <div>
          <label>Zip: </label>
          <Field as={TextField} style={{ height: '25px', margin: '4px' }} name="zip" />
...
const CheckoutApp = withFormik({
  ...
  validationSchema: Yup.object().shape({
    fullName: Yup.string().required('Full name is mandatory for submission.'),
    email: Yup.string().email('Email not valid.'),
    street: Yup.string().required('Must enter a billing address.'),
    city: Yup.string().required('Must enter a billing address.'),
    state: Yup.string().required('Must enter a billing address.'),
    zip: Yup.string().required('Must enter a billing address.'),
    paymentMethod: Yup.string().required('Must select a payment method.'),
  }),
