...
const seed = async () => {
  try {
    ...
    const populateCartInstances = (num, users, products) => {
      const carts = [];

      for (let i = 0; i < num; i++) {
        const randUser = Math.floor(Math.random() * 5);
        const randProduct = Math.floor(Math.random() * 100);
        const singleUser = users[randUser];
        const singleProduct = products[randProduct];
        const randQuantity = Math.ceil(Math.random() * 5);
        const cartInstance = {
    ...
    const carts = populateCartInstances(5, allUsers, allProducts);
